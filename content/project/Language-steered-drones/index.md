...
The pipeline acquires real-world imagery of an environment, reconstructs a **3D Gaussian Splat (GSplat) representation**, and derives a **semantic scene abstraction**. The demonstration shows **low-level motor rate control** using **RRT-based trajectory tracking** along a spiral path in a **3D Gaussian Splat reconstructed environment**. Policies are trained in simulation and transferred to real-world experiments, enabling **language-driven navigation from perception to control**.
